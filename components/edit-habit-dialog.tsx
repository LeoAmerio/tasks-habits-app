...
import { useHabitContext, type Habit, type HabitFrequency, type HabitGoal } from "@/context/habit-context"
...
export function EditHabitDialog({ open, onOpenChange, habit }: EditHabitDialogProps) {
  const { updateHabit, sections, addSection } = useHabitContext()
  const [name, setName] = useState(habit.name)
  const [frequency, setFrequency] = useState<HabitFrequency>(habit.frequency)
  const [selectedDays, setSelectedDays] = useState<number[]>(habit.selectedDays || [0, 1, 2, 3, 4, 5, 6])
  const [goal, setGoal] = useState<HabitGoal>(habit.goal)
  const [startDate, setStartDate] = useState<Date>(habit.startDate)
  const [endDate, setEndDate] = useState<Date | undefined>(habit.endDate)
  const [section, setSection] = useState<string>(habit.section)
  const [autoPopup, setAutoPopup] = useState(habit.autoPopup)
